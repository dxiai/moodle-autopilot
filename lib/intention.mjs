...
export class IntentionScript {
    ...
    async prepareConnection(token) {
        ...
        // TODO Dynamic system Loading

        this.api = await moodle.initMoodle(system.url, token);
    }
    ...
    async parseAllIntentions() {
        ...
        this.workflow = await Promise.all(workflow.map(
            async (intent, id) => {
                ...
                const myclass = await import(`./intentions/${typeClass}.mjs`);
                
                Intentions[typeClass] = myclass[typeClass];
