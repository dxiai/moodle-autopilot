...
export class Assignment extends IntentBase {
    ...
    setApi(api) {
        super.setApi(api);
        if (!api.mod_assign.get.submissions) {
            throw new IntentionError({
                message: "API not enabled: mod_assign_get_submissions",
                intention: `${this.type}: ${this.name} (${this.id})`
            });
        }
        if (!api.mod_assign.get.assignments) {
            throw new IntentionError({
                message: "API not enabled: mod_assign_get_assignments",
                intention: `${this.type}: ${this.name} (${this.id})`
            });
        }
        if (!api.mod_assign.save.grade) {
            throw new IntentionError({
                message: "API not enabled: mod_assign_save_grade",
                intention: `${this.type}: ${this.name} (${this.id})`
            });
        }
    }
